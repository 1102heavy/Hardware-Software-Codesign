...
#define DMA_DEV_ID_0        XPAR_XAXIDMA_0_BASEADDR
#define DMA_DEV_ID_1        XPAR_XAXIDMA_1_BASEADDR
#define DMA_DEV_ID_2        XPAR_XAXIDMA_2_BASEADDR
...
XAxiDma *DMAInstancePtr0 = &AxiDma0; // Device pointer

XAxiDma AxiDma1;	// Device instance
XAxiDma *DMAInstancePtr1 = &AxiDma1; // Device pointer

XAxiDma AxiDma2;	// Device instance
XAxiDma *DMAInstancePtr2 = &AxiDma2; // Device pointer
...
#endif
{
	...
	CfgPtr = XAxiDma_LookupConfig(DMA_DEV_ID_0);
	if (!CfgPtr) {
	  xil_printf("No config found for %d\r\n", DMA_DEV_ID_0);
	  return XST_FAILURE;
	}
	...
	CfgPtr = XAxiDma_LookupConfig(DMA_DEV_ID_1);
	if (!CfgPtr) {
	  xil_printf("No config found for %d\r\n", DMA_DEV_ID_1);
	  return XST_FAILURE;
	}
	...
	/* Initialize the XAxiDma2 device.*/
	CfgPtr = XAxiDma_LookupConfig(DMA_DEV_ID_2);
	if (!CfgPtr) {
	  xil_printf("No config found for %d\r\n", DMA_DEV_ID_2);
	  return XST_FAILURE;
	}

	Status = XAxiDma_CfgInitialize(&AxiDma2, CfgPtr);
	if (Status != XST_SUCCESS) {
	  xil_printf("Initialization failed %d\r\n", Status);
	  return XST_FAILURE;
	}
	...
	while(Running)
	{
		...
		xil_printf(" Transmitting Data for DMA_0(HDL)\r\n");
		...
		t0 = XTmrCtr_GetValue(TmrCtrInstancePtr, TIMER_COUNTER_0);

		Status = XAxiDma_SimpleTransfer(&AxiDma0,(u32) (result_memory), 4*NUMBER_OF_OUTPUT_WORDS, XAXIDMA_DEVICE_TO_DMA);//rx

		if (Status != XST_SUCCESS) {
		  return XST_FAILURE;
		}
		
		Status = XAxiDma_SimpleTransfer(&AxiDma0,(u32) (test_input_memory), 4*NUMBER_OF_INPUT_WORDS, XAXIDMA_DMA_TO_DEVICE);//tx
		...
		t1 = XTmrCtr_GetValue(TmrCtrInstancePtr, TIMER_COUNTER_0);
		u32 cycles_dma = t1 - t0;   // unsigned wraparound-safe
		
		uart_puts(UART_BASEADDR, "CYC_DMA_0(HDL)=");
		...
		int dma_success = 1;
		for (int r = 0; r < A_ROWS; r++){
			if(result[r]!=*(result_memory + r)){
				xil_printf("DMA0 Calculation failed\n\r");
				dma_success = 0;
				break;
			}
		}
		if (dma_success){
			xil_printf("DMA0 Calculation Success\n\r");
		}

		xil_printf(" Transmitting Data for DMA_1(HLS)\r\n");
		...
		Status = XAxiDma_SimpleTransfer(&AxiDma1,(u32) (result_memory), 4*NUMBER_OF_OUTPUT_WORDS, XAXIDMA_DEVICE_TO_DMA);//rx

		if (Status != XST_SUCCESS) {
		  return XST_FAILURE;
		}
		
		Status = XAxiDma_SimpleTransfer(&AxiDma1,(u32) (test_input_memory), 4*NUMBER_OF_INPUT_WORDS, XAXIDMA_DMA_TO_DEVICE);//tx
		...
		t1 = XTmrCtr_GetValue(TmrCtrInstancePtr, TIMER_COUNTER_0);
		cycles_dma = t1 - t0;   // unsigned wraparound-safe
		
		uart_puts(UART_BASEADDR, "CYC_DMA_1(HLS)=");
		uart_put_u32_dec(UART_BASEADDR, cycles_dma);
		uart_puts(UART_BASEADDR, "\n");
		uart_puts(UART_BASEADDR, "\r");
		uart_puts(UART_BASEADDR, "\n");
		...
		for (int r = 0; r < A_ROWS; r++){
			if(result[r]!=*(result_memory + r)){
				xil_printf("DMA1 Calculation failed\n\r");
				dma_success = 0;
				break;
			}
		}
		if (dma_success){
			xil_printf("DMA1 Calculation Success\n\r");
		}

		xil_printf(" Transmitting Data for DMA_2(HLS Optimised)\r\n");
		...
		Status = XAxiDma_SimpleTransfer(&AxiDma2,(u32) (result_memory), 4*NUMBER_OF_OUTPUT_WORDS, XAXIDMA_DEVICE_TO_DMA);//rx

		if (Status != XST_SUCCESS) {
		  return XST_FAILURE;
		}
		
		Status = XAxiDma_SimpleTransfer(&AxiDma2,(u32) (test_input_memory), 4*NUMBER_OF_INPUT_WORDS, XAXIDMA_DMA_TO_DEVICE);//tx
		...
		uart_puts(UART_BASEADDR, "CYC_DMA_2(HDL Optimised)=");
		uart_put_u32_dec(UART_BASEADDR, cycles_dma);
		uart_puts(UART_BASEADDR, "\n");
		uart_puts(UART_BASEADDR, "\r");
		uart_puts(UART_BASEADDR, "\n");
		...
		for (int r = 0; r < A_ROWS; r++){
			if(result[r]!=*(result_memory + r)){
				xil_printf("DMA2 Calculation failed\n\r");
				dma_success = 0;
				break;
			}
		}
		if (dma_success){
			xil_printf("DMA2 Calculation Success\n\r");
		}
